CLASS z2ui5_cl_demo_app_160 DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
...
CLASS z2ui5_cl_demo_app_160 IMPLEMENTATION.
  ...
  METHOD on_event.
    ...
    IF mo_client->check_on_event( `PL_TOTAL_CHANGE` ).
      ...
      mo_client->message_box_display( lv_tab_index && lv_id_event && lv_id_parent ).
    ENDIF.
  ...
  METHOD render_main_screen.
    ...
    DATA(lo_table) = lo_page->flex_box( height = `85vh` )->ui_table( alternaterowcolors  = `true`
                                                               visiblerowcountmode = `Auto`
                                                               fixedrowcount       = `1`
                                                               selectionmode       = `None`
                                                               rows                = mo_client->_bind_edit( val = mt_output
*        compress_mode = z2ui5_if_client=>cs_compress_mode-none
      ) ).
    DATA(lo_columns) = lo_table->ui_columns( ).

    lo_columns->ui_column( width          = `5.2rem`
                        sortproperty   = `SET_SK`
    ...
    lo_columns->ui_column( width          = `20rem`
    ...
    lo_columns->ui_column( width          = `5rem`
      ...
      value           = `{PL_TOTAL}`
      submit          = mo_client->_event( val = `PL_TOTAL_CHANGE` t_arg = VALUE #(
        ( `${$source>/id}` )
        ( `${INDEX}` )
*        ( `$source.oParent.sId` )
    ...
    lo_columns->ui_column( width          = `4rem`
