  ...
  METHOD display_view.
    ...
    lo_page_01->vertical_layout(
              width = `100%`
                                   ...
                                   active = abap_true
                                   state  = `Error`
                                   press  = mo_client->_event( `handleStatusPressed` )
    ...
    lo_page_01->vertical_layout( class = `sapUiContentPadding`
                              width = `100%`
               )->block_layout( background = `transparent`
                   )->block_layout_row(
                       )->block_layout_cell(
  ...
  METHOD on_event.
    ...
    CASE mo_client->get( )-event.
      ...
      WHEN `handleStatusPressed`.
        mo_client->message_box_display( title   = `Error description`
